...
def mobile(request, data=None):
    if data == None:
        ...
        mobiles = Product.objects.filter(
            category="M").filter(discounted_price__lte=10000)
    elif data == "above":
        mobiles = Product.objects.filter(
...
def laptop(request, data=None):
    if data == None:
        laptops = Product.objects.filter(category="L")
    elif data == "Macbook" or data == "Dell" or data == "Zenbook":
        laptops = Product.objects.filter(category="L").filter(brand=data)
    elif data == "below":
        laptops = Product.objects.filter(
            category="L").filter(discounted_price__lte=40000)
    elif data == "above":
        laptops = Product.objects.filter(
...
def topwear(request, data=None):
    if data == None:
        ...
        topwears = Product.objects.filter(
            category="TW").filter(discounted_price__lte=400)
    elif data == "above":
        topwears = Product.objects.filter(
...
def bottomwear(request, data=None):
    if data == None:
        ...
        bottomwears = Product.objects.filter(
            category="BW").filter(discounted_price__lte=400)
    elif data == "above":
        bottomwears = Product.objects.filter(
...
class CustomerRegistrationView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            form.save()
            messages.success(
                request, f"Congratulations!!! Registered Successfully.")
        return render(request, 'app/customerregistration.html', {"form": form})
